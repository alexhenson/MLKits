...
class LogisticRegression {
  ...
  recordCost() {
    const cost = tf.tidy(() => {
      ...
      const termOne = this.labels.transpose().matMul(guesses.add(1e-7).log()); // add a constant to void log(0)

      const termTwo = this.labels
        .mul(-1)
        .add(1)
        .transpose()
        .matMul(guesses.mul(-1).add(1).add(1e-7).log()); // 1 x 10 ^ -7 = 0.00000001
